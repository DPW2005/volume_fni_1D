...
public class GaussSeidelSolver {
    ...
    public double[] solve(double[][] A, double[] b, double[] solutionInitial, int maxIterations, double tolerance) {
        if (A == null || b == null || solutionInitial == null) {
            throw new IllegalArgumentException("Les entrées ne peuvent pas être nulles.");
        }
        int n = A.length;
        if (n == 0 || A[0].length != n || b.length != n || solutionInitial.length != n) {
            throw new IllegalArgumentException("Dimensions de matrice/vecteur incompatibles.");
        }

        double[] x = solutionInitial.clone(); // Utilise une copie de l'estimation initiale
